...
describe("MemoryCache", () => {
  ...
  it("salva e recupera um valor dentro do TTL", () => {
    ...
    expect(memoryCache.get("jobs:react")).toEqual([{ titulo: "Dev React" }]);
  });

  it("remove e retorna null quando o item expirou", () => {
    const nowSpy = vi.spyOn(Date, "now");
    nowSpy.mockReturnValueOnce(1000);
    memoryCache.set("jobs:node", [{ titulo: "Dev Node" }], 5000);
  ...
  it("aplica TTL mínimo de 1000ms", () => {
    const nowSpy = vi.spyOn(Date, "now");
    nowSpy.mockReturnValueOnce(1000);
    memoryCache.set("jobs:vue", [{ titulo: "Dev Vue" }], 0);
    nowSpy.mockReturnValueOnce(1500);
    expect(memoryCache.get("jobs:vue")).toEqual([{ titulo: "Dev Vue" }]);
  });
