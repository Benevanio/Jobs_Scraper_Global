...
describe("cache facade", () => {
  ...
  it("exporta cache como MemoryCache quando REDIS_URL não está configurada", async () => {
    const { cache } = await import("../../../../src/cache/cache.js");
    const { MemoryCache } =
      await import("../../../../src/cache/memoryCache.js");
    expect(cache).toBeInstanceOf(MemoryCache);
  });

  it("exporta cache como RedisCache quando REDIS_URL está configurada", async () => {
    process.env.REDIS_URL = "redis://127.0.0.1:6379";
    const { client } = buildRedisClientMock();
    mocks.createClient.mockReturnValue(client);
  ...
  it("warmupCache retorna status memory quando REDIS_URL não está configurada", async () => {
    const { warmupCache } = await import("../../../../src/cache/cache.js");
  ...
  it("warmupCache retorna connected true após ping bem-sucedido", async () => {
    process.env.REDIS_URL = "redis://127.0.0.1:6379";
    const { client } = buildRedisClientMock({ ping: "PONG" });
    mocks.createClient.mockReturnValue(client);
  ...
  it("warmupCache emite warn quando ping falha", async () => {
    process.env.REDIS_URL = "redis://127.0.0.1:6379";
    const { client } = buildRedisClientMock();
    ...
    expect(mocks.consoleWarn).toHaveBeenCalledWith(
      expect.stringContaining("Falha ao validar conexão com Redis"),
      expect.any(String),
    );
  });
});
